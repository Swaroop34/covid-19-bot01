...
def processRequest(req):
    ...
    if intent == 'state_wise_covid19_cases':
        ...
        for i, j in state_list.items():
            if j == 'district':
                for get_district_name in j:
                    if get_district_name == district_name:
                        confirmed_cases = get_district_name['confirmed']
                        active_cases = get_district_name['active']
                        deaths = get_district_name['deceased']
                        recovered = get_district_name['recovered']
